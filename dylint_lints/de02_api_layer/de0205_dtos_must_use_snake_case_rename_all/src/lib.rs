...
dylint_linting::declare_pre_expansion_lint! {
    /// DE0205: DTOs Must Use Snake Case in Serde Attributes
    ///
    ...
    ///
    /// Only snake_case is allowed for API consistency.
    pub DE0205_DTOS_MUST_USE_SNAKE_CASE_RENAME_ALL,
    Deny,
    "DTOs must use snake_case in serde rename attributes"
}

impl EarlyLintPass for De0205DtosMustUseSnakeCaseRenameAll {
...
fn check_type_rename_all(cx: &EarlyContext<'_>, attrs: &[Attribute]) {
    for (span, value) in find_serde_attribute_value(attrs, "rename_all") {
        if value != "snake_case" {
            cx.span_lint(
                DE0205_DTOS_MUST_USE_SNAKE_CASE_RENAME_ALL,
                span,
                |diag| {
                    diag.primary_message(
                        "DTOs must not use non-snake_case in serde rename_all (DE0205)"
                    );
        ...
        for (span, value) in find_serde_attribute_value(&field.attrs, "rename") {
            if !is_snake_case(&value) {
                cx.span_lint(
                    ...
                    |diag| {
                        diag.primary_message(
                            "DTO fields must not use non-snake_case in serde rename (DE0205)"
                        );
